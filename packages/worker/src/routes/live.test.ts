...
describe("GET /api/live", () => {
	...
	test("critical alert → 503, status: critical", async () => {
		const now = Math.floor(Date.now() / 1000);
		await insertHost(db, "host-001", now);
	...
	test("offline detection → 503, counted as critical", async () => {
		const now = Math.floor(Date.now() / 1000);
	...
	test("zero hosts → 503, status: empty", async () => {
		const res = await get(app);
		expect(res.status).toBe(503);
		const body = (await res.json()) as HealthResponse;
	...
	test("retired hosts excluded from health check", async () => {
		const now = Math.floor(Date.now() / 1000);
