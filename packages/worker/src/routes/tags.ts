...
import { MAX_TAGS_PER_HOST, TAG_COLOR_COUNT, TAG_MAX_LENGTH } from "@bat/shared";
...
export async function tagsListRoute(c: Context<AppEnv>) {
	const rows = await c.var.repos.tags.list();
	return c.json(rows);
}
...
export async function tagsCreateRoute(c: Context<AppEnv>) {
	...
	const result = await c.var.repos.tags.create(validated.name, validated.color);
	if (result.ok === "duplicate") {
		return c.json({ error: "Tag with this name already exists" }, 409);
	}
	return c.json({ ...result.row, host_count: 0 }, 201);
}
...
export async function tagsUpdateRoute(c: Context<AppEnv, "/api/tags/:id">) {
	...
	const result = await c.var.repos.tags.update(tagId, {
...
export async function tagsDeleteRoute(c: Context<AppEnv, "/api/tags/:id">) {
	...
	const removed = await c.var.repos.tags.delete(tagId);
	if (!removed) {
		return c.json({ error: "Tag not found" }, 404);
	}
...
export async function tagsByHostsRoute(c: Context<AppEnv>) {
	return c.json(await c.var.repos.tags.byHostsAll());
}
...
export async function hostTagsListRoute(c: Context<AppEnv, "/api/hosts/:id/tags">) {
	const hostId = c.var.connectServerId ?? c.req.param("id");
...
export async function hostTagsAddRoute(c: Context<AppEnv, "/api/hosts/:id/tags">) {
	...
	const result = await c.var.repos.tags.addToHost(hostId, validated.tag_id);
	if (result.ok === "host_not_found") {
		return c.json({ error: "Host not found" }, 404);
	}
	if (result.ok === "tag_not_found") {
		return c.json({ error: "Tag not found" }, 404);
	}
...
export async function hostTagsReplaceRoute(c: Context<AppEnv, "/api/hosts/:id/tags">) {
	...
	const result = await c.var.repos.tags.replaceForHost(hostId, validated.tag_ids);
	if (result.ok === "host_not_found") {
		return c.json({ error: "Host not found" }, 404);
	}
	if (result.ok === "tags_not_found") {
		return c.json({ error: `Tags not found: ${result.missing.join(", ")}` }, 404);
	}
...
export async function hostTagsRemoveRoute(c: Context<AppEnv, "/api/hosts/:id/tags/:tagId">) {
	...
	const removed = await c.var.repos.tags.removeFromHost(hostId, tagId);
	if (!removed) {
