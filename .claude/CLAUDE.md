...
- **Release baseline snapshot 已纳入 `scripts/release.ts`**：`VERSION_TARGETS` 现在会同步 `packages/worker/test/e2e/baseline/__snapshots__/{live,fleet.status}.json`。不要再手工 `--update` + amend。
- **Stale cargo-llvm-cov artifacts look like 88%**：先用 stable 跑过 llvm-cov、或 nightly 的 `coverage(off)` 尚未编进产物时，probe 会报 ~88% 并卡住 95% 门禁。`cargo +nightly llvm-cov clean` 后再跑，真实行覆盖约 99%。
- **Dummy `packages/worker/static/index.html` breaks L2 SPA**：为了启动 wrangler 随手写的 `<!doctype html><title>bat</title>` 会让 `GET / returns SPA HTML` 失败。pre-push 前必须 `bun turbo build --filter=@bat/ui`。
- **Playwright seed 标题不能带当前 app 版本**：`Deploy v2.1.0` 会被 `release.ts` 的 stale-version `rg` 当成漏改版本号并中止发版。夹具改用固定的 `v1.2.3`。
